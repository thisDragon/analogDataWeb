...
public class HttpRequest {
	...
	* @throws
	...
	public static String httpClientGetOfPartition(String url, Map<String, String> paramMap) throws ClientProtocolException, IOException{
        ...
        //将回调函数信息中的例如下
        //0000keshihua.operation.Dictionary.m_getPicleftList({"count":17,"code":1,"dataDictList":[{}]})进行切割
        if (httpEntityContent.indexOf(UrlUtils.getDataDicTreeRun)!=-1) {
        	httpEntityContent = httpEntityContent.split(UrlUtils.getDataDicTreeRun)[1];
		}else if (httpEntityContent.indexOf(UrlUtils.insertTeamWorkRun)!=-1) {
			httpEntityContent = httpEntityContent.split(UrlUtils.insertTeamWorkRun)[1];
		}else if (httpEntityContent.indexOf(UrlUtils.queryResouseInfoRun)!=-1) {
			httpEntityContent = httpEntityContent.split(UrlUtils.queryResouseInfoRun)[1];
		}
        httpGet.abort();
        return httpEntityContent.replaceAll("\\(", "").replaceAll("\\)", "");
	}
